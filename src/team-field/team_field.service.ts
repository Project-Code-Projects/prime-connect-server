...
export class TeamFieldService {
    constructor( @Inject('TEAM_FIELD_REPOSITORY') private teamFieldRepository: typeof TeamField) {}

    async createTeamField(createTeamFieldDto: any): Promise<TeamField> {
        const teamField = await this.teamFieldRepository.create<TeamField>(createTeamFieldDto);
        return teamField;
    }
